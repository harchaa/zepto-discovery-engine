...
| Three-tier tagging workload: skip off-topic Reddit entirely, auto-tag "trivial" reviews (short, no friction/category/avoidance keyword) deterministically with zero LLM cost, reserve the LLM only for the "substantive" pool, capped at a target of 20,000 tagged | Keep sending every non-off-topic record through the LLM and just accept the multi-day timeline | Hit **both** Groq models' daily token quotas (100K for the 70B chatbot model, and a similarly hard cap on the 8B tagging model) within a single day of combined pipeline + testing usage. Measured that 54% of the cleaned corpus (34,481 off-topic + 67,662 trivial-praise reviews like "good", "nice app") needed zero LLM judgment — sending them anyway was pure waste, since off-topic records get discarded at analysis time and trivial ones have an obvious, keyword-verifiable answer. The 20,000 target was a deliberate choice, not a technical ceiling: enough for confident frequency counts on even the rarer category-exploration signal (~8-10% base rate → 1,600-2,000 expected examples), reached with the user's explicit sign-off after being shown the exact volume breakdown | The "trivial" blocklist is a judgment call — a short review could theoretically contain missed nuance the keyword list doesn't catch. Kept deliberately conservative (blocklist of ~60 signal keywords spanning friction/category/avoidance terms, not just an English praise-word allowlist, so it also safely handles Hinglish short praise) and length-capped at 40 chars, erring toward sending anything ambiguous to the LLM rather than auto-tagging it |
| Re-test and reinstate Apple App Store as a data source after a human review caught a wrong earlier conclusion | Trust the original test result ("confirmed dead platform-wide, verified against Instagram too") and leave the corpus at two sources | A round-2 review flagged that the exact same endpoint returns real data — re-tested and confirmed: 450 real reviews came back immediately, for both Zepto and Instagram. The original test's result was almost certainly a transient outage/caching blip on Apple's side, not a dead feed; one failed probe (even cross-checked) isn't strong enough evidence to call something permanently unavailable | Worth naming honestly: this was a real analysis error, caught by review rather than caught proactively. The fix is cheap (the scraper already existed in spirit, just needed writing) and the corpus is now genuinely three-source, restoring the third cross-triangulation leg §3 originally called for |
