...
| App Store scraping hits a platform ceiling | Could be mistaken for a scraper bug rather than an expected ceiling | **Confirmed during execution: worse than a cap — the public RSS reviews feed now returns zero entries for any app (verified against Instagram too), and the modern web API requires a developer-account bearer token we don't have. Documented as unavailable; corpus is Play Store + Reddit only.** |
