...
| **Round 2 spot-check (20 fresh records tagged under the corrected prompt):** the staple-category mistag was gone, but found two more problems: (a) the `stated_avoidance` phrase list was too narrow, wrongly stripping real avoidance statements phrased as "don't ever order" or "stopped using it"; (b) Reddit posts merely name-dropping "Zepto" without being about an actual shopping experience (a tech-interview-topics joke, a stock-market comment) were still getting a friction tag, because `off_topic_flag` only checks whether the word "zepto" appears, not whether the post is about the shopping experience | Confirms round-1 fixes held for the main failure mode, but a fix can have gaps of its own — worth explicitly re-checking rather than assuming "fixed once" means "fixed" | **Expanded the avoidance phrase list (added "don't ever", "stopped using", "don't use", etc.) and added `looks_off_topic()` — a Reddit-only check that resets friction tags to `none` if the text contains none of a broad commerce-keyword list. A full corpus scan (not just the 20-record sample) found and corrected 57 more off-topic mistags. One over-correction was caught in turn (a genuine on-topic personal-care question got reset) and the fix now preserves pre-correction tags (`pre_off_topic_correction`) specifically so a keyword-list refinement can self-correct without repeating this — 5 records from the first, narrower pass had no such backup and were dropped for fresh re-tagging instead.** |
